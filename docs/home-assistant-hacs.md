...
**Completed task:** Task 06A — Device-scoped config entries and atomic identity-registry migration — committed as `cc4bb75`; verified: `mise exec python -- python -m pytest -q -W error tests/components/larapaper_bridge` — 209 passed; managed Ruff — all checks passed; Pyright — 0 errors, 0 warnings, 0 informations.
**Completed task:** Task 06B — Per-entry runtime, entity, and diagnostic isolation — committed as `b784024`; verified: `mise exec python -- python -m pytest -q -W error tests/components/larapaper_bridge` — 213 passed; touched-file Ruff — all checks passed; production Pyright — OK.
**Completed task:** Task 06C — Fair shared image admission across devices — committed as `176edaf`, `8de5f3f`, `928a4c6`; verified: `mise exec python -- python -m pytest -q -W error tests/components/larapaper_bridge/test_image.py` — 103 passed; full integration suite — 217 passed; touched-file Ruff — all checks passed; active per-entry private-origin union and unload/remove revocation covered.
**Completed task:** Task 06D — Coalesced manual display cycle and native button — committed as `edf94e4`, `70055c2`; verified: `mise exec python -- python -m pytest -q -W error tests/components/larapaper_bridge` — 224 passed; touched-file Ruff — all checks passed; production Pyright — OK; `git diff --check` — clean.
**Status:** blocked; blocked: ARCHIVE has no repository-defined destination for completed local backlog markdown; tried: inspected the current tree, repository instructions, and complete docs path history, and the sole move to `docs/archive/home-assistant-hacs.md` was immediately restored because that location was invented; unblock: identify an existing repository archive convention or obtain an explicit repository-owner decision establishing the destination; remaining: archive `docs/home-assistant-hacs.md` only; HA-HACS-06 acceptance 1–9 and review remain complete.
**Review marker:** reviewed: `cc4bb75`, `b784024`, `176edaf`, `8de5f3f`, `928a4c6`, `edf94e4`, `70055c2` [review-fix: `89dd332`]; verified: `mise exec python -- python -m pytest -q -W error tests/components/larapaper_bridge` — 224 passed; production Pyright — 0 errors, 0 warnings, 0 informations; changed Python Ruff — all checks passed.
...
5. **06B:** Camera registry tests prove the stable MAC-derived unique ID, one device-registry record per MAC, friendly name and `Larapaper` manufacturer, exact per-entry diagnostics lookup, byte-identical cache-only camera reads, stale equality, and zero network/state mutation from camera or diagnostics reads. The runtime-owned MAC-scoped device identity is reusable by the later button platform.
...
7. **06D:** Scheduler/button tests prove the button's stable MAC-derived unique ID and shared device-registry record, an idle press starts exactly one immediate cycle, concurrent presses coalesce, a press during an active scheduled/manual display adds no call, no two display calls overlap, and button code never invokes the client directly.
