...
**Task 06A — Device-scoped config entries and forward-only identity migration (acceptance 1–3):** Remove `single_config_entry` from the manifest, set config-flow `VERSION = 2`, and replace the constant domain unique ID with the canonical MAC, so one config entry represents exactly one device and duplicate MACs are rejected before entry creation. Keep the existing user form and per-device URL/poll/image settings. Title a pending entry with its MAC; after complete credentials are loaded or provisioned, update its title to `friendly_id` through `hass.config_entries.async_update_entry`. Replace the fixed Store key with deterministic path-safe `larapaper_bridge.<mac12>`, where `<mac12>` is the canonical MAC lowercased with colons removed, while retaining the exact pending/complete payload schema. Add `async_migrate_entry`: for the sole V1 entry, validate the legacy `larapaper_bridge` payload and matching entry MAC, save it to the device-scoped Store first, update the entry version/unique ID/title through `hass.config_entries.async_update_entry`, then remove the legacy Store. A failure before legacy removal leaves the validated legacy source intact and returns migration failure; retries are idempotent when the new payload already matches. A conflicting new payload, malformed legacy state, or MAC mismatch fails closed without deletion. New flows never inspect or mutate the legacy key. `async_remove_entry` deletes only the removed entry's device-scoped Store; unload/reload never deletes it. Generated MACs are checked against configured entry unique IDs and regenerated on collision; a supplied duplicate aborts without Store mutation.
...
1. Use repeated config entries, one per device. Home Assistant's config-entry lifecycle natively sets up, unloads, removes, and migrates entries independently; the current V1 runtime, camera, and diagnostics are already entry-shaped. Config subentries would retain a shared parent failure/removal boundary, while a domain map would require a second lifecycle model.
2. Use canonical MAC as config-entry unique ID and Store-key suffix. The MAC is already the persisted Larapaper identity and is available before entry creation, so it gives duplicate detection and interrupted-flow recovery without inventing another identifier. Entry IDs remain HA-local lifecycle keys; they are not device identity.
3. Migrate forward by copy-validate-update-delete. Home Assistant calls `async_migrate_entry` for version changes and requires `async_update_entry` rather than direct mutation. Saving the new exact payload before removing the legacy Store gives an idempotent retry boundary. Device removal uses the documented `async_remove_entry` hook and intentionally deletes only that device's credentials.
...
**Open questions resolved:** All seven draft questions are settled above: active display calls coalesce; admitted manual cycles reset cadence from settlement; timed-out manual calls use `display_failed` with no fast retry; devices use repeated config entries; the button remains available during cold/stale/error only after provisioning; Store migration is forward-only, idempotent, and per-device removal deletes only its own state; shared conversion becomes FIFO-admitted before payload fetch. “Multiple devices” and “manual next display” remain one item because the button's stable identity, lifecycle lookup, and isolation depend on the multi-entry conversion, while tasks 06A–06D provide bounded implementation passes.

**Acceptance:**

1. **06A:** Config-flow tests create two entries with distinct canonical MAC unique IDs and independent settings, reject supplied/generated collisions before Store mutation, and prove interrupted pending flows reuse only their own device-scoped Store.
2. **06A:** Migration tests cover complete and pending V1 payloads, save-before-delete ordering, entry version/unique-ID/title updates, idempotent retry after partial completion, and fail-closed malformed/mismatched/conflicting states with the legacy payload intact.
3. **06A:** Removal/restart tests prove removing one entry deletes only `larapaper_bridge.<MAC>`, while unload/reload and Home Assistant restart preserve every remaining identity and complete credentials without another setup call.
...
**Biggest non-obvious risk:** A “multi-device” refactor that changes Store keys but leaves a singleton runtime lookup can pass isolated tests while unloading one entry cancels or redirects another entry's late completion. Every lookup, callback, timer, entity, diagnostics read, and token check must carry the owning entry ID and MAC explicitly.
